...
public class Main {
	...
	public static void main(String[] args) {
		...
		// Scenario 5- no living cells
		ArrayList<Cell> scenario5Cells = new ArrayList<Cell>();
		main.playGame(5, scenario5Cells, main);

		// Initial seed Scenario 6
	...
	/**
	 * Pass in cell array and number of iterations and will iterate cell array that
	 * many times
	 */
	public void playGame(int numberOfTimes, ArrayList<Cell> cells, Main main) {
		// Run an extra iteration to print initial and final grid
	...
	public ArrayList<Cell> iterate(ArrayList<Cell> cells, Main main) {
		...
		// After making grid, iterate over the old live cells to remove any which will
		// now be dead
			...
			for (int y = 0; y < grid[x].length; y++) {
				...
				if (grid[x][y] == 0) {
					...
					// case: top of grid
						...
						totalNeighbours = grid[x][y + 1] + grid[x][y - 1] + grid[x - 1][y] + grid[x - 1][y + 1]
								+ grid[x - 1][y - 1];
					}
					// Case: Side of grid
					else if (y == grid[x].length - 1) {
						totalNeighbours = grid[x + 1][y] + grid[x - 1][y] + grid[x][y - 1] + grid[x + 1][y - 1]
								+ grid[x - 1][y - 1];
					}
					// case: bottom of grid
					else if (x == 0) {
	...
	/**
	 * Calculate the number of neighbours each cell in an array of cells at a
	 * certain x,y have Given the first and last x and y positions of all the cells
	 * and the list of cells Will print out the grid of the cell positions
	 */
	public void calcNeighbourNumber(int firstXPos, int lastXPos, int firstYPos, int lastYPos, ArrayList<Cell> cells) {
		...
		// Array is looped through in a way that will make x and y match up to standard
		// axis placements i.e. ^ y > x
		...
		for (int y = finalGrid[0].length - 1; y > -1; y--) {
			for (int x = 0; x < finalGrid.length; x++) {
				if (finalGrid[x][y] instanceof Cell) {
					...
					// case: corners of grid
					if (x == 0 && y == 0) {
						if (finalGrid[x + 1][y] instanceof Cell)
							totalNeighbours++;
						if (finalGrid[x][y + 1] instanceof Cell)
							totalNeighbours++;
						if (finalGrid[x + 1][y + 1] instanceof Cell)
							totalNeighbours++;
		...
		// Print grid showing initial result (before iteration and cells added)
